...
export function ArticleCard({ article, priority = false }: ArticleCardProps) {
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-[1.75rem] border border-[var(--pub-border)] bg-[var(--pub-surface)] shadow-[0_16px_50px_var(--pub-card-hover-shadow)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_24px_80px_var(--pub-card-hover-shadow)]">
      <Link href={`/articles/${article.slug}`} className="block">
        <div className="relative aspect-[16/11] overflow-hidden bg-[var(--pub-bg)]">
          <ManagedImage
      ...
      <div className="flex flex-1 flex-col space-y-3 p-5">
        ...
        <Link href={`/articles/${article.slug}`} className="block flex-1">
